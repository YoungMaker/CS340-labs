...
(defn apply-to-seq-helper [f y s x]
  (if (empty? s)
     x
     (recur f y (rest s) (conj x (f (first s) y)))))

 
(defn apply-to-sequence-the-hard-way [f y s]
  
 (apply-to-seq-helper f y s []))
...
;(defn swap [input] ;swap first two items in list, return new list
 ; (vector (second x) (first x)))
...
(defn swapify [s]
  (loop [ss s
    ...
    (if (empty? ss)
      ...
      (if (empty? (rest ss))
        (conj acc (first ss))
        (recur (rest (rest ss)) (conj acc (second ss) (first ss)))))))
