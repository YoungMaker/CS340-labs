...
(defn tally-item [prices line-item]
  (let [item (first line-item)
        qty (second line-item)
        price (get prices item)]
    ( * qty price)
    ))
...
(defn invoice-total [prices invoice]
   (reduce + (map (fn [line-item] (tally-item prices line-item)) invoice))
  )
